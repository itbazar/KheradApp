...
export function getHandlerTableChange(setQueryParams) {
  return (type, { page, sizePerPage, sortField, sortOrder, data }) => {
    ...
    setQueryParams((prev) =>
      type === "sort"
        ? { ...prev, sortOrder, sortField,orderCluase:`${sortField} ${sortOrder}` }
        : type === "pagination"
...
export function PleaseWaitMessage({ entities }) {
  return <>{entities === null && <div><FormattedMessage id="WAIT.MESSAGE" /></div>}</>;
}
  ...
  return (
    <>
      {customersList.length === 0 && entities !== null && (
        <div><FormattedMessage id="PAGINATION.NO.RECORD" /></div>
      )}
    </>
  );
}
